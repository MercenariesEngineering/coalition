(function (factory) {
  ...
  function SlickColumnPicker(columns, grid, options) {
    ...
    var defaults = {
      ...
      formatters: {
        ...
        group: function (column, groups) {
          ...
          var picker = '<label>';
          picker += '<input id="picker-group-' + column.id + '" type="checkbox" class="picker-group" title="Grouped/Not Grouped" data-column-id="' + column.id + '" ' + (columnGrouped ? 'checked="checked"' : '') + ' >';
          picker += '</label>';
          return picker;
        },
        total: function (column, totals) {
          ...
          var picker = '<label>';
          picker += '<input id="picker-total-' + column.id + '" type="checkbox" class="picker-total" title="Totalized/Remover Total" data-column-id="' + column.id + '" ' + (columnGrouped ? 'checked="checked"' : '') + ' >';
          picker += '</label>';
          return picker;
        }
      }
    ...
    function picker(showIconLevel, column) {
      ...
      if (column.groupable && Slick.Data.GroupItemMetadataProvider && grid.getData() instanceof Slick.Data.DataView)
        picker += options.formatters.group(column, options.groups);
      else picker += '<span>&nbsp;&nbsp;&nbsp;&nbsp;</span>';
...
column.total=0;
      var field = column.field,
        totals = {};
      if (column.totalizationType === Slick.Controls.ColumnPicker.Countrows) {
	count=0;
      $.each(items, function (item) {
	if (items[item][field] == column.totalizationvalue ||column.totalizationvalue == 'ALL' )
	        ++count;
      });

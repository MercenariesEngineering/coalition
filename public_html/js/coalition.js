...
function initJobs() {
    var columns = [
        ...
        {id: "title", name: "Title", field: "title", sortable: true, formatter:formattertitle, totalized: true, totalizationType:Slick.Controls.ColumnPicker.Countrows, totalizationvalue: 'ALL'},
        ...
        {id: "timeout", name: "timeout", field: "timeout", cssClass: 'slick-right-align'},
...
function gridJobsSaveConfig() {
	// Save grid configuration in browser local storage
	console.log("Saving config");
	localStorage.setItem("gridJobsColumns", JSON.stringify(gridjobs.getColumns()));
}

function gridJobsGetConfig() {
	// Get stored grid configuration
	console.log("Getting config");
	try {
		config = JSON.parse(localStorage.getItem("gridJobsColumns"));
	} catch(e) {
		config = null;
	}
	return config;
...
function gridJobsLoadConfig() {
	// Load grid configuration
	config = gridJobsGetConfig();
	console.log("Loading config: " + config);
	if (config) {
		gridjobs.setColumns(config);
	}
}
...
function getsqlwhereJobs(emitter_id) {
	// Get Jobs matching the request using batch
	var max_batch = numberofrowsdisplayed()
    var where_clause = document.getElementById(emitter_id).value;
    $("#pagination").empty();
    if (!where_clause) {
        reloadJobs();
        return;
	...
	.done(function(total) {
		if (total) {
			if (total <= max_batch) {
				data = {where_clause: where_clause, min: 0, max: 1000000000};
				getsqlwhereJobsresults(data);
			} else {
				var batches = Math.round(total / max_batch);
				for (i = 0; i <= batches; i++) {
					min = i * max_batch;
					max = (i + 1) * max_batch - 1;
					data = {where_clause: where_clause, min: min, max: max};
					button = '<button type="button" onclick="getsqlwhereJobsresults($(this).data())">'+min+'-'+max+'</button>'
	...
	.always(function(xhr, status) {
		console.log("Finished where clause.");
	});
}
function getsqlwhereJobsresults(data) {
	$.ajax({
		type: "GET",
		url: "/api/jobs/where/",
		data: data,
